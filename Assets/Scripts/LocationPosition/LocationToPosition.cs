...
public class LocationToPosition : MonoBehaviour
{
    ...
    void PlaceImageAtTargetLocation()
    {
        ...
        {
            Debug.LogError("Reference latitude or longitude is null.");
            return; // 또는 예외를 발생시키거나 적절한 처리
        }
        ...
        if (latDiff == null || lonDiff == null)
        {
            Debug.LogError("Latitude or Longitude difference is null.");
            return; // 또는 예외를 발생시키거나 적절한 처리
        }

        // 지구 반경을 이용하여 x, z 값 계산
        double z = latDiff.Value * (Mathf.PI / 180) * earthRadius;
        double x = lonDiff.Value * (Mathf.PI / 180) * earthRadius * System.Math.Cos(referenceLatitude.Value * Mathf.PI / 180);
